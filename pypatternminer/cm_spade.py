#!/usr/bin/env python3
"""
Python implementation of the CM-SPADE algorithm for sequential pattern mining with abstractions, based on the Java code from the original paper.
"""
...
class Item:
    ...
    def __lt__(self, other: "Item") -> bool:
        # Descending order to mimic Java's negative compareTo.
...
class AbstractionCreatorQualitative(AbstractionCreator):
    ...
    def generate_candidates(self, creator: "AbstractionCreator", pattern1: Pattern, pattern2: Pattern, min_support: float) -> List[Pattern]:
        """Generate all possible candidates following the Java CM-SPADE logic."""
        candidates: List[Pattern] = []
        elements1 = pattern1.get_elements()
        elements2 = pattern2.get_elements()
        prefix_len = len(elements1) - 1
        ...
        last1 = pattern1.get_last_element()
        last2 = pattern2.get_last_element()
        if last1 is None or last2 is None:
            return candidates
        abs1 = last1.abstraction
        abs2 = last2.abstraction
        ...
        if not eq1 and not eq2:
            if last1.item != last2.item:
                if last1.item.id <= last2.item.id:
                    base_pattern = pattern1
                    other_item = last2.item
                else:
                    base_pattern = pattern2
                    other_item = last1.item
                c_equal = base_pattern.clone_pattern()
                c_equal.add(pair_creator.get_item_abstraction_pair(other_item, qual_creator.create_abstraction_flag(True)))
                candidates.append(c_equal)

                c_before_changed = pattern2.clone_pattern()
                c_before_changed.add(last1)
                candidates.append(c_before_changed)
            c_before = pattern1.clone_pattern()
            c_before.add(last2)
            candidates.append(c_before)
        elif eq1 and eq2:
            new_candidate: Optional[Pattern] = None
            if pattern1 < pattern2:
                new_candidate = pattern1.clone_pattern()
                new_candidate.add(pair_creator.get_item_abstraction_pair(last2.item, qual_creator.create_abstraction_flag(True)))
            elif pattern1 > pattern2:
                new_candidate = pattern2.clone_pattern()
                new_candidate.add(pair_creator.get_item_abstraction_pair(last1.item, qual_creator.create_abstraction_flag(True)))
            ...
            if eq1:
                new_candidate = pattern1.clone_pattern()
                new_candidate.add(last2)
            else:
                new_candidate = pattern2.clone_pattern()
                new_candidate.add(last1)
            if new_candidate is not None:
                candidates.append(new_candidate)
...
class CandidateGeneration:
    def __init__(self) -> None:
        ...
        self.cooc_after: Dict[int, Dict[int, int]] = {}
        self.cooc_equals: Dict[int, Dict[int, int]] = {}

    def set_cooc_maps(self, after_map: Dict[int, Dict[int, int]], equals_map: Dict[int, Dict[int, int]]) -> None:
        self.cooc_after = after_map
        self.cooc_equals = equals_map

    def _passes_cooc(self, candidate: Pattern, min_support_absolute: float) -> bool:
        if candidate.size() < 2:
            return True
        last = candidate.get_last_element()
        prev = candidate.get_last_but_one_element()
        if last is None or prev is None:
            return True
        if isinstance(last.abstraction, AbstractionQualitative) and last.abstraction.equal_relation:
            a, b = prev.item.id, last.item.id
            count = self.cooc_equals.get(a, {}).get(b, 0)
        else:
            a, b = prev.item.id, last.item.id
            count = self.cooc_after.get(a, {}).get(b, 0)
        return count >= min_support_absolute
    def __init__(self) -> None:
        self.last_raw_candidate_count: int = 0
        ...
        if k > 2:
                ...
                for i in range(len(group)):
                    for j in range(i, len(group)):
                        for candidate in abstraction_creator.generate_candidates(
                            ...
                            if self._passes_cooc(candidate, min_support_absolute):
            ...
            for i in range(len(frequent_list)):
                for j in range(i, len(frequent_list)):
                    for cand in abstraction_creator.generate_size2_candidates(abstraction_creator, frequent_list[i], frequent_list[j]):
                        if self._passes_cooc(cand, min_support_absolute):
...
def build_cooc_maps(sequences: List[Sequence]) -> Tuple[Dict[int, Dict[int, int]], Dict[int, Dict[int, int]]]:
    """Build co-occurrence maps used to prune candidate generation (after / equals)."""
    after_map: Dict[int, Dict[int, int]] = {}
    equals_map: Dict[int, Dict[int, int]] = {}
    for sequence in sequences:
        # positions of each item (item id -> set of itemset indices)
        item_positions: Dict[int, Set[int]] = {}
        for idx, itemset in enumerate(sequence.itemsets):
            for item in set(itemset.items):
                item_positions.setdefault(item.id, set()).add(idx)

        # equals: items appearing in the same itemset
        counted_equals: Set[Tuple[int, int]] = set()
        for itemset in sequence.itemsets:
            items = sorted({it.id for it in itemset.items})
            for i in range(len(items)):
                for j in range(i + 1, len(items)):
                    a, b = items[i], items[j]
                    key = (a, b)
                    if key in counted_equals:
                        continue
                    counted_equals.add(key)
                    equals_map.setdefault(a, {}).setdefault(b, 0)
                    equals_map[a][b] += 1

        # after: item b appearing in a later itemset than item a
        counted_after: Set[Tuple[int, int]] = set()
        items_list = list(item_positions.items())
        for a, pos_a in items_list:
            for b, pos_b in items_list:
                if any(pb > pa for pa in pos_a for pb in pos_b):
                    key = (a, b)
                    if key in counted_after:
                        continue
                    counted_after.add(key)
                    after_map.setdefault(a, {}).setdefault(b, 0)
                    after_map[a][b] += 1
    return after_map, equals_map
...
class AlgoCMSPADE:
    ...
    def run_algorithm(
        ...
        after_map, equals_map = build_cooc_maps(database.get_sequences())
        candidate_generator.set_cooc_maps(after_map, equals_map)
...
def main(output_path: Optional[Path] = None) -> None:
    ...
    algo = AlgoCMSPADE(support, mingap, maxgap, window_size, abstraction_creator)
    output_file = output_path or Path(__file__).with_name("cmspade_output.txt")
    if output_file.exists():
        output_file.unlink()

    algo.run_algorithm(sequence_database, keep_patterns, verbose, str(output_file), output_sequence_identifiers)

    print(f"Relative Minimum support = {support}")
