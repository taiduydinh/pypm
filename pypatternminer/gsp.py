#!/usr/bin/env python3
"""
Python implementation of the GSP implementation.
"""
...
class AbstractionCreator:
    ...
    def generate_candidates(self, creator: "AbstractionCreator", pattern1: Pattern, pattern2: Pattern, min_support: float) -> Optional[Pattern]:
        raise NotImplementedError
...
class AbstractionCreatorQualitative(AbstractionCreator):
    ...
    def generate_candidates(self, creator: "AbstractionCreator", pattern1: Pattern, pattern2: Pattern, min_support: float) -> Optional[Pattern]:
        ...
        for i in range(len(elements1) - 1):
            pair1 = elements1[i + 1]
            pair2 = elements2[i]
            if i == 0:
                if pair1.item != pair2.item:
                    different = True
            else:
                if pair1 != pair2:
                    different = True
        ...
        if different:
            return None
        intersection = pattern1.appearing_in & pattern2.appearing_in
        if len(intersection) >= min_support:
            new_pattern = pattern1.clone_pattern()
            last_element = pattern2.get_last_element()
            if last_element is not None:
                new_pattern.add(last_element)
            return new_pattern
        return None
...
class CandidateGeneration:
    def generate_candidates(
        ...
        if k > 2:
            previous_item: Optional[Item] = None
            matching: Optional[Set[Pattern]] = None
            for frequent_pattern1 in frequent_list:
                current_item = frequent_pattern1.get_ith_element(1).item
                if previous_item is None or current_item != previous_item:
                    matching = indexation_map.get(current_item)
                    previous_item = current_item
                if matching:
                    for frequent_pattern2 in matching:
                        candidate = abstraction_creator.generate_candidates(abstraction_creator, frequent_pattern1, frequent_pattern2, min_support_absolute)
                        if candidate is not None:
                            candidate_set.append(candidate)
            if not candidate_set:
                return None
            pruned_candidates = self._pruned_subset(candidate_set, frequent_set, abstraction_creator)
        elif k == 2:
            pruned_candidates = []
...
def main(output_path: Optional[Path] = None) -> None:
    ...
    output_file = output_path or Path(__file__).with_name("gsp_outputs.txt")
    ...
    print()
    print(f"{algo.get_number_of_frequent_patterns()} frequent pattern found.")
